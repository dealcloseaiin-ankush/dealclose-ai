...
export const TriggerNode = ({ id, data }) => {
  ...
  return (
    <div className="bg-[#111] p-4 rounded-2xl shadow-2xl border border-emerald-500 min-w-[260px] text-white relative group">
      {id !== '1' && (
        <button onClick={() => {
          setNodes(nds => nds.filter(n => n.id !== id));
          setEdges(eds => eds.filter(e => e.source !== id && e.target !== id));
        }} className="absolute top-2 right-2 text-gray-400 hover:text-rose-500 p-1 transition-opacity"><X size={16}/></button>
      )}
      <div className="font-bold mb-3 flex items-center gap-2 text-emerald-400 text-sm">🚀 Start Trigger</div>
      <select value={triggerType} onChange={handleTriggerTypeChange} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-emerald-500 mb-3">
...
export const MessageNode = ({ id, data, selected, templates = [], ...props }) => {
  ...
  return (
    <div className={`bg-[#111] p-4 rounded-2xl shadow-2xl border min-w-[280px] text-white relative group ${isInstagram ? 'border-pink-500' : 'border-blue-500'}`}>
      ...
      <div className={`font-bold mb-3 flex items-center gap-2 text-sm ${isInstagram ? 'text-pink-400' : 'text-blue-400'}`}>
      ...
      <div className="space-y-3">
        ...
        {!isInstagram && (
          <div>
            ...
            <select defaultValue={data?.template || ""} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, template: e.target.value } } : n)))} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-blue-500">
        ...
        <div>
          ...
          <textarea defaultValue={data?.message || data?.label || ""} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, message: e.target.value } } : n)))} className={`nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white placeholder-gray-600 ${isInstagram ? 'focus:border-pink-500' : 'focus:border-blue-500'}`} rows="2" placeholder="Hi there! How can we help?"></textarea>
        </div>
      </div>
...
export const AskQuestionNode = ({ id, data, selected }) => {
  ...
  return (
    <div className="bg-[#111] p-4 rounded-2xl shadow-2xl border border-purple-500 min-w-[280px] text-white relative group">
      ...
      <div className="font-bold mb-3 flex items-center gap-2 text-purple-400 text-sm">⚡ Ask Question (Wait for Reply)</div>
      <div className="space-y-3">
        <div>
          <p className="text-[11px] text-gray-400 mb-1">Expected Reply Type</p>
          <select value={replyType} onChange={handleReplyTypeChange} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-purple-500 mb-2">
        ...
        <div>
          ...
          <textarea defaultValue={data?.question || data?.label || ""} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, question: e.target.value } } : n)))} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-purple-500 placeholder-gray-600" rows="2" placeholder="e.g., What is your name?"></textarea>
        </div>
        
        {replyType === 'yes_no' && (
        <div>
          <p className="text-[11px] text-gray-400 mb-1">Expected Replies (Branches)</p>
          <div className="flex justify-between text-[10px] font-bold px-2 mt-3 bg-[#1a1a1a] p-2 rounded-xl border border-gray-800">
        ...
        {replyType === 'open' && (
        <div>
          <div className="flex justify-center text-xs font-bold px-2 mt-3 bg-[#1a1a1a] p-2 rounded-xl border border-gray-800">
...
export const DelayNode = ({ id, data, selected }) => {
  const { setNodes, setEdges } = useReactFlow();
  return (
    <div className="bg-[#111] p-4 rounded-2xl shadow-2xl border border-gray-500 min-w-[220px] text-white relative group">
      ...
      <div className="font-bold mb-3 flex items-center gap-2 text-gray-300 text-sm">⏳ Wait / Delay</div>
      <div className="flex gap-2">
        <input type="number" onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, delay: e.target.value } } : n)))} className="nodrag nopan w-20 bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-gray-400" defaultValue={data?.delay || "15"} />
        <select defaultValue={data?.unit || "Minutes"} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, unit: e.target.value } } : n)))} className="nodrag nopan flex-1 bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-gray-400">
...
export const ConditionNode = ({ id, data, selected }) => {
  const { setNodes, setEdges } = useReactFlow();
  return (
    <div className="bg-[#111] p-4 rounded-2xl shadow-2xl border border-orange-500 min-w-[250px] text-white relative group">
      ...
      <div className="font-bold mb-3 flex items-center gap-2 text-orange-400 text-sm">🔄 Condition (If/Else)</div>
      <select defaultValue={data?.condition || "If User Replied"} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, condition: e.target.value } } : n)))} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-orange-500">
        <option>If User Replied</option>
        <option>If Payment Pending</option>
        <option>If Tag = VIP</option>
      </select>
      <div className="flex justify-between mt-4 text-xs font-bold px-2">
...
export const MenuNode = ({ id, data, selected }) => {
  const { setNodes, setEdges } = useReactFlow();
  return (
    <div className="bg-[#111] p-4 rounded-2xl shadow-2xl border border-teal-500 min-w-[280px] text-white relative group">
      ...
      <div className="font-bold mb-3 flex items-center gap-2 text-teal-400 text-sm"><ListPlus size={18} /> Interactive Menu</div>
      <div className="space-y-3">
        <div>
          <p className="text-[11px] text-gray-400 mb-1">Message Text</p>
          <textarea defaultValue={data?.message || ""} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, message: e.target.value } } : n)))} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-teal-500 placeholder-gray-600" rows="2" placeholder="Please select an option:"></textarea>
        </div>
        <div>
          <p className="text-[11px] text-gray-400 mb-1">Button 1</p>
          <input type="text" defaultValue={data?.opt1 || ""} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, opt1: e.target.value } } : n)))} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-teal-500" placeholder="e.g. Collab / PR" />
        </div>
        <div>
          <p className="text-[11px] text-gray-400 mb-1">Button 2</p>
          <input type="text" defaultValue={data?.opt2 || ""} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, opt2: e.target.value } } : n)))} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-teal-500" placeholder="e.g. Paid Ads" />
        </div>
        <div>
          <p className="text-[11px] text-gray-400 mb-1">Button 3</p>
          <input type="text" defaultValue={data?.opt3 || ""} onChange={(e) => setNodes((nds) => nds.map((n) => (n.id === id ? { ...n, data: { ...n.data, opt3: e.target.value } } : n)))} className="nodrag nopan w-full bg-[#1a1a1a] border border-gray-700 rounded-xl p-2.5 text-xs outline-none text-white focus:border-teal-500" placeholder="e.g. Just a Fan ❤️" />
        </div>
      </div>
