...
const MetaConnectButton = ({ buttonText = 'Connect WhatsApp', platform = 'whatsapp', workspaceId = 'main', onSuccess }) => {
  ...
  useEffect(() => {
    console.log("?? [Meta SDK] Checking SDK status...");
    ...
    if (!document.getElementById('facebook-jssdk')) {
      console.log("?? [Meta SDK] Injecting Facebook SDK script...");
      window.fbAsyncInit = function () {
        console.log("?? [Meta SDK] fbAsyncInit called, initializing...");
        window.FB.init({
          appId: APP_ID,
          cookie: true,
          xfbml: true,
  ...
  const handleMetaLogin = () => {
    console.log("?? [MetaConnect] Button clicked. isSdkLoaded status:", isSdkLoaded);
    if (!window.FB) {
      console.warn("?? [MetaConnect] window.FB is not available yet.");
      alert('Meta SDK is loading, please wait a second...');
      return;
    }

    if (!APP_ID) {
      alert("?? VITE_META_APP_ID is missing! Please add it to your frontend/.env file.");
      return;
    }

    // Failsafe Initialization just in case it missed it
    if (!window.isFbInitialized) {
      console.log("?? [MetaConnect] Failsafe: Initializing FB SDK now...");
      window.FB.init({
        appId: APP_ID,
        cookie: true,
        xfbml: true,
    ...
    setLoading(true);
    console.log("?? [MetaConnect] Triggering FB.login popup...");

    // Trigger Meta Oauth Popup
    const handleLoginResponse = (response) => {
      console.log("?? [MetaConnect] FB.login response received:", response);
    ...
    console.log('?? [MetaConnect] Sending authCode to backend API...');
    ...
    .then(res => {
      ...
      if (platform === 'instagram' && res.data.availableAccounts) {
         ...
         return;
          ...
          const data = res.data;
          console.log('?? [MetaConnect] Backend API response:', data);
          if (data.success) {
